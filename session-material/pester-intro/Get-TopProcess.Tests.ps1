...
#region starter test
#these tests are running the full command
...
Describe "Get-TopProcess" {
    ...
    It "Should accept a value for the number of processes" {
        (Get-TopProcess -count 1).Count | Should -BeExactly 1
    }
    It "Should accept a computername as a parameter value." {
        <Test> | Should -Be "YOUR_EXPECTED_VALUE"
    } -skip
...
#endregion
<#
#this test is in the tests2 file for demo purposes
